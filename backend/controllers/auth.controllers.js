...
export const register = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      "INSERT INTO usuarios (nombre, email, contrasenia, fecha_registro) VALUES ($1, $2, $3, $4)",
      [nombre, email, hashPassword, fecha_registro]
    );
...
export const refreshToken = async (req, res) => {
  ...
  try {
    const decoded = jwt.verify(refreshToken, REFRESH_SECRET_KEY);
    const result = await pool.query('SELECT id_usuario FROM usuarios WHERE id_usuario = $1', [decoded.id]);

    if (result.rows.length === 0) {
      return res.status(401).json({ message: 'Usuario no válido' });
    }

    const token = jwt.sign({ id: result.rows[0].id_usuario }, SECRET_KEY, { expiresIn: '1h' });
